...
class Qwen3_VQA_Batch(Qwen3BaseNode):
    ...
    def INPUT_TYPES(s):
        ...
        return {
                "required": {
                    "images": ("IMAGE", {"tooltip": "输入的批量图片列表 [B, H, W, C]。可连接 ComfyUI 的 Load Image Batch 或类似的 List 节点。"}),
                    "prompt_template": (sorted(prompt_files), {"default": prompt_files[0] if prompt_files else "", "tooltip": "选择提示词模板文件，将应用于 Batch 中的每一张图片。"}),
                    "model": ([
                        "Huihui-Qwen3-VL-8B-Instruct-abliterated",
                        "Qwen3-VL-8B-Instruct", "Qwen3-VL-4B-Instruct",
                        "Qwen3-VL-8B-Thinking", "Qwen3-VL-4B-Thinking",
                    ...
                    "quantization": (["none", "4bit", "8bit"], {"default": "none", "tooltip": "选择模型量化方式。对于 24G 显存，推荐使用 'none' (BF16) 以获得最佳质量，或 '4bit' 以获得极致速度。"}),
                    "keep_model_loaded": ("BOOLEAN", {"default": False, "tooltip": "如果开启，模型在推理完成后将保留在显存中，下次运行将非常快。"}),
                    "batch_size": ("INT", {"default": 4, "min": 1, "max": 32, "tooltip": "显卡并行处理的图片数量。对于 3090 (24G)，建议设为 8-12。设置过大会导致 OOM。"}),
                    "temperature": ("FLOAT", {"default": 0.7, "tooltip": "控制生成文本的随机性。"}),
                    "max_new_tokens": ("INT", {"default": 768, "tooltip": "每张图片允许生成的最大 Token 数。注意：这是单张图的限制，不是总和。"}),
                    "min_pixels": ("INT", {"default": 256 * 28 * 28, "tooltip": "处理图片时的最小总像素数。"}),
                    "max_pixels": ("INT", {"default": 768 * 28 * 28, "tooltip": "处理图片时的最大总像素数。超过此值的图片会被自动缩小。"}),
                    "seed": ("INT", {"default": -1, "tooltip": "随机种子。"}),
                    "attention": (["sage_attention_2", "flash_attention_2", "sdpa", "eager"], {"default": "sage_attention_2", "tooltip": "选择注意力机制实现。"}),
                    "performance_mode": (["balanced", "speed", "quality"], {"default": "balanced", "tooltip": "性能模式预设。"}),
                },
                "optional": {
                    "user_prompt": ("STRING", {"default": "", "multiline": True, "tooltip": "用户输入的辅助提示词，将应用于 Batch 中的每一张图片。"}),
                },
            }
    RETURN_TYPES = ("STRING",) # ComfyUI standard type
    RETURN_NAMES = ("captions",)
    OUTPUT_IS_LIST = (True,) # 标记输出为列表
    FUNCTION = "run_inference_batch"
    CATEGORY = "Comfyui_Qwen3-VL-Instruct"

    def run_inference_batch(self, images, prompt_template, model, keep_model_loaded, batch_size,
                           temperature, max_new_tokens, min_pixels, max_pixels, seed, quantization,
                           user_prompt="", attention="sage_attention_2", performance_mode="balanced", **kwargs):
        # 1. 读取模板
        template_content = ""
        try:
            with open(os.path.join(self.prompts_dir, prompt_template), 'r', encoding='utf-8') as f:
        ...
        # 2. 准备图片列表 (内存转换 + 预 Resize)
        pil_images_list = self._tensor_to_pil(images)
        # 预先调整图片大小，防止超大图导致 OOM (Processor 虽然有 max_pixels，但预处理阶段可能仍耗费大量显存)
        # 简单的逻辑：如果总像素超过 max_pixels，按比例缩小
        resized_images = []
        for img in pil_images_list:
            ...
            if total_pixels > max_pixels:
                ...
                # print(f"[Batch Resize] {w}x{h} -> {new_w}x{new_h}")
            resized_images.append(img)
        pil_images_list = resized_images # 使用 Resize 后的列表
        total_images = len(pil_images_list)
        all_captions = []
        # 3. 资源获取 & 种子处理
        self.manager.acquire(self.session_id)
        try:
            # 加载模型
            self.manager.load_model(model, quantization, attention, min_pixels, max_pixels)
            model_instance = self.manager.model
            processor = self.manager.processor
            actual_seed, _ = validate_seed(seed)
            torch.manual_seed(actual_seed)
            # 4. 分批处理 (Mini-batch)
            for i in range(0, total_images, batch_size):
                batch_imgs = pil_images_list[i : i + batch_size]
                current_batch_size = len(batch_imgs)
                print(f"[Qwen3_VQA_Batch] Processing batch {i//batch_size + 1}, size: {current_batch_size}")
                # 构造 Batch Prompt: 每个样本都是 [System + User(Image+Text)]
                # 注意：Transformers 的 Qwen2VL 处理器支持批量输入，只需传入 list of messages
                batch_messages = []
                ...
                # Padding: Qwen2VL 需要 left padding for generation
                ...
                start_t = time.time()
                with torch.no_grad():
                    generated_ids = model_instance.generate(**inputs, **gen_kwargs)
                print(f"[Qwen3_VQA_Batch] Batch inference time: {time.time() - start_t:.2f}s")
                # 解码
                generated_ids_trimmed = [
                    out_ids[len(in_ids):] for in_ids, out_ids in zip(inputs.input_ids, generated_ids)
                ]
                ...
                # 格式化为单行文本
                batch_outputs = [" ".join(text.split()) for text in batch_outputs]
                all_captions.extend(batch_outputs)
            return (all_captions,)
        finally:
            self.manager.release(self.session_id, keep_model_loaded)
# 注册节点
NODE_CLASS_MAPPINGS = {
    ...
    "Qwen3_VQA_Batch": Qwen3_VQA_Batch,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "Qwen3_VQA": "Qwen3 VQA",
    "Qwen3_VQA_Quick": "Qwen3 VQA Quick",
    "Qwen3_VQA_Batch": "Qwen3 VQA Batch",
}
